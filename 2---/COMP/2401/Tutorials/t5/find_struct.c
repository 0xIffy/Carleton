...
int cmpEmployee(struct emp *, char *);
struct emp * findEmployee(struct emp**, int, char*);
...
int main(int argc, char* argv[])
{
	...
	for (i = 0; i < MAX_EMPLOYEES; i++) {
		...
		*(empArr+i) = (struct emp *) malloc(sizeof(struct emp));
		...
		if (empArr+i == NULL) {
			printf("error: could not allocate memory for the array element \n");
			return(1);
		}
...
struct emp * findEmployee(struct emp** arr, int arrSize, char* familyName){
	printf("\n");
	for(int i = 0; i < arrSize; i++, ++arr){
		if(cmpEmployee(*arr,familyName)){
			printf("%s %s\n",(*arr)->firstName,(*arr)->familyName);
			printf("salary = %.2f\tyears of service = %.2f\n",(*arr)->salary,(*arr)->yearsWithCompany);
			return *arr;
		}
	}
...

{
	...
	// add code
	// use the -> operatortor to access the fields
