...
function submitOrder(){
	...
	req.open("POST", `https://localhost:3000/orders`);
...
function getMenuHTML(rest){
	...
	Object.keys(menu).forEach(key =>{
		...
		Object.keys(menu[key]).forEach(id => {
			item = menu[key][id];
			result += `${item.name} (\$${item.price}) <img src='imgs/add.jpg' style='height:20px;vertical-align:bottom;' onclick='addItem(${id})'/> <br>`;
...
function updateOrder(){
	...
	Object.keys(order).forEach(id =>{
		...
		result += `${item.name} x ${order[id].quantity} (${(item.price * order[id].quantity).toFixed(2)}) <img src='imgs/remove.jpg' style='height:15px;vertical-align:bottom;' onclick='removeItem(${id})'/><br>`;
	});
