...
bool ResManager::isAvailable(Room *r, Date *d, int sty){
	...
	for(int i = 0; i < numRes; i++){
		if(reservations[i]->getRoom()->getNum() == r->getNum()){
			...
			if((d->lessThan(temp2) && (reservations[i]->getDate())->lessThan(*d)) ||((reservations[i]->getDate())->lessThan(temp1) && (d->lessThan(*reservations[i]->getDate()) || d->equals(*reservations[i]->getDate())))){
